// RPOW2 CUDA-Powered Miner - Single Account
// Uses a custom CUDA SHA-256 kernel for GPU acceleration.
// Pipeline overlap: pre-fetch next challenge while submitting current.
//
// Build the solver first:
//   cd solver-cuda && ./build.sh

const { execFile } = require('child_process');
const path = require('path');
const config = require('./config');

// Path to compiled CUDA solver binary.
const BINARY_NAME = process.platform === 'win32' ? 'rpow2-cuda.exe' : 'rpow2-cuda';
const SOLVER_BIN = process.env.RPOW_SOLVER_BIN ||
  path.join(__dirname, 'solver-cuda', BINARY_NAME);

// Nonces per kernel launch (16M default). Override via RPOW_BATCH.
const BATCH_SIZE = parseInt(process.env.RPOW_BATCH || (1 << 24).toString(), 10);
...
function solveWithCuda(noncePrefixHex, difficultyBits) {
  return new Promise((resolve, reject) => {
    const args = [noncePrefixHex, String(difficultyBits), String(BATCH_SIZE)];

    // Generous timeout — at 10+ GH/s a difficulty-33 solve is sub-second on
    // average, but PoW variance can still produce long tails on bad luck.
    const SOLVE_TIMEOUT_MS = 10 * 60 * 1000;

    const solveStart = Date.now();
    let resolved = false;

    // Heartbeat every 10s (CUDA solves are usually fast, but show progress).
    const heartbeat = setInterval(() => {
      if (resolved) return;
      const elapsedSec = Math.floor((Date.now() - solveStart) / 1000);
      const m = Math.floor(elapsedSec / 60);
      const s = elapsedSec % 60;
      process.stdout.write(`  [gpu solving...] elapsed ${m}m ${s}s (difficulty=${difficultyBits})\n`);
    }, 10_000);
    ...
    execFile(SOLVER_BIN, args, {
      ...
      if (error) {
        reject(new Error(`CUDA solver failed: ${error.message}\nstderr: ${stderr}`));
        return;
      }

      const solutionNonce = stdout.trim();
      if (!solutionNonce || !/^\d+$/.test(solutionNonce)) {
        reject(new Error(`Invalid CUDA solver output: "${stdout.trim()}"\nstderr: ${stderr}`));
        return;
      }
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  RPOW2 CUDA-Powered Miner (GPU)');
  console.log(`  Solver: ${SOLVER_BIN}`);
  console.log(`  Batch:  ${BATCH_SIZE.toLocaleString()} nonces/launch`);
  console.log(`  API:    ${config.API_BASE}`);
  console.log('═══════════════════════════════════════════════════');
  console.log();
  console.log('  ⚠  WARNING: GPU mining may trigger anti-bot detection on');
  console.log('     rpow2.com and violate your GPU cloud provider\'s ToS.');
  ...
  try {
    ...
    console.log('  ✓ CUDA solver binary found');
  } catch {
    console.error(`  ✗ CUDA solver binary not found at: ${SOLVER_BIN}`);
    console.error('  Build it: cd solver-cuda && ./build.sh');
    process.exit(1);
  }
  ...
  while (true) {
    try {
      ...
      const solveStart = Date.now();
      const result = await solveWithCuda(nonce_prefix, difficulty_bits);
