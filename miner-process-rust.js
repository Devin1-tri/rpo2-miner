// RPOW2 Single-Account Miner Process (Rust-Powered)
// Spawned by multi.js - communicates stats via IPC (process.send)
// Uses native Rust binary instead of worker_threads
//
// T40 fork changes:
...
const NUM_THREADS = parseInt(process.env.MINER_WORKERS || '50', 10);
...
async function main() {
  ...
  try {
    ...
    sendStats({ email, balance, mined: 0, hashrate: 0, lastSolveTime: 0, status: 'ready' });
  } catch (err) {
  ...
  while (true) {
    try {
      sendStats({ email, balance, mined: totalMinted, hashrate: 0, lastSolveTime: 0, status: 'fetching' });
      ...
      sendStats({ email, balance, mined: totalMinted, difficulty: difficulty_bits, status: 'mining' });

      const solveStart = Date.now();
      const result = await solveWithRust(nonce_prefix, difficulty_bits);
      const solveTime = Date.now() - solveStart;

      sendStats({ email, balance, mined: totalMinted, hashrate: result.hashrate, lastSolveTime: solveTime, difficulty: difficulty_bits, status: 'submitting' });

      // Submit mint
      const mintResult = await submitMint(challenge_id, result.nonce);

      // Pre-fetch next challenge
      ...
      retries++;
      sendStats({ email, balance, mined: totalMinted, status: 'error', error: err.message });
